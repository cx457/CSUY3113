...
// Jump on the enemies to kill them. Three enemies:One Waitandgo . One Patroller. One Jumper.
...
void Render() {
    ...
    if (state.player->win == true) {
        DrawText(&program, font, "You Win", 1, -0.5f, glm::vec3(-4.25f, 1, 0));
    }
    else if (state.player->failed == true) {
        for (int i = 0; i < ENEMY_COUNT; i++) {
            state.enemies[i].aiState = TERMINATE;
        }
        DrawText(&program, font, "You Lose", 1, -0.5f, glm::vec3(-3.25f, 1, 0));
