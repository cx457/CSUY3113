...
// Jump on the enemies to kill them. Two Sound Effect: One occurs when player wins the whole game, another
// occurs when player loses the game. If you fall down, restart is the only solution.:(
// Player only dies when they got touched by AI. If a player die and still has lives, 
//the player will go back to the born position.Thanks for playing!
...
void Update() {
    ...
    if (sceneTrack == 4 && currentScene->state.player->win == true && playSuccessTime == 1) {
